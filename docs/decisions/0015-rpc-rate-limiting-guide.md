...
**Problème :** Le transport `http()` utilise les défauts viem — pas de config de retry, pas de batch, pas de polling interval custom. Le RPC public rate-limite agressivement.
...
| Fichier | Rôle |
|---------|------|
| `lib/wagmi.ts` | Config RPC + transport |
